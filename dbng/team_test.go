	...
	FDescribe("SavePipeline", func() {
		...
		Context("on initial create", func() {
			...
			It("updates resource config", func() {
				...
				savedPipeline, _, err := team.SavePipeline(pipelineName, config, 1, dbng.PipelineNoChange)
				Expect(err).NotTo(HaveOccurred())
		...
		Context("on updates", func() {
			...
			It("updating from unpaused to paused", func() {
				...
				_, _, configVersion, err := team.GetConfig(pipelineName)
				Expect(err).NotTo(HaveOccurred())

				_, _, err = team.SavePipeline(pipelineName, config, configVersion, dbng.PipelinePaused)
				Expect(err).NotTo(HaveOccurred())
			...
			Context("updating with no change", func() {
				It("maintains paused if the pipeline is paused", func() {
					...
					_, _, err = team.SavePipeline(pipelineName, config, configVersion, dbng.PipelineNoChange)
					Expect(err).NotTo(HaveOccurred())
		...
		It("can manage multiple pipeline configurations", func() {
			...
			initialConfig, _, _, err := team.GetConfig(pipelineName)
			Expect(err).NotTo(HaveOccurred())
			Expect(initialConfig).To(BeZero())
			initialOtherConfig, _, _, err := team.GetConfig(otherPipelineName)
			Expect(err).NotTo(HaveOccurred())
			...
			returnedConfig, returnedRawConfig, configVersion, err := team.GetConfig(pipelineName)
			Expect(err).NotTo(HaveOccurred())
			...
			otherReturnedConfig, otherReturnedRawConfig, otherConfigVersion, err := team.GetConfig(otherPipelineName)
			Expect(err).NotTo(HaveOccurred())
			...
			_, _, err = team.SavePipeline(pipelineName, updatedConfig, configVersion-1, dbng.PipelineUnpaused)
			Expect(err).To(Equal(dbng.ErrConfigComparisonFailed))

			_, _, err = team.SavePipeline(pipelineName, updatedConfig, configVersion+10, dbng.PipelineUnpaused)
			Expect(err).To(Equal(dbng.ErrConfigComparisonFailed))

			_, _, err = team.SavePipeline(otherPipelineName, updatedConfig, otherConfigVersion-1, dbng.PipelineUnpaused)
			Expect(err).To(Equal(dbng.ErrConfigComparisonFailed))

			_, _, err = team.SavePipeline(otherPipelineName, updatedConfig, otherConfigVersion+10, dbng.PipelineUnpaused)
			Expect(err).To(Equal(dbng.ErrConfigComparisonFailed))

			By("being able to update the config with a valid con")
			_, _, err = team.SavePipeline(pipelineName, updatedConfig, configVersion, dbng.PipelineUnpaused)
			Expect(err).NotTo(HaveOccurred())
			_, _, err = team.SavePipeline(otherPipelineName, updatedConfig, otherConfigVersion, dbng.PipelineUnpaused)
			Expect(err).NotTo(HaveOccurred())

			By("returning the updated config")
			returnedConfig, returnedRawConfig, newConfigVersion, err := team.GetConfig(pipelineName)
			Expect(err).NotTo(HaveOccurred())
			...
			otherReturnedConfig, _, newOtherConfigVersion, err := team.GetConfig(otherPipelineName)
			Expect(err).NotTo(HaveOccurred())
			...
			_, _, err = team.SavePipeline(invalidPipelineName, config, 1, dbng.PipelineUnpaused)
			Expect(err).NotTo(HaveOccurred())
		...
		`)
			...
			_, _, invalidConfigVersion, err := team.GetConfig(invalidPipelineName)
		...
		Context("when there are multiple teams", func() {
			var otherTeamDB dbng.TeamDB

			BeforeEach(func() {
				_, err := database.CreateTeam(dbng.Team{Name: "some-other-team"})
				Expect(err).NotTo(HaveOccurred())

				otherTeamDB = teamFactory.GetTeamDB("some-other-team")
			})

			It("can allow pipelines with the same name across teams", func() {
				_, _, err := team.SavePipeline("steve", config, 0, dbng.PipelineUnpaused)
				Expect(err).NotTo(HaveOccurred())

				By("allowing you to save a pipeline with the same name in another team")
				_, _, err = otherTeamDB.SavePipeline("steve", otherConfig, 0, dbng.PipelineUnpaused)
				Expect(err).NotTo(HaveOccurred())

				By("getting the config for the correct team's pipeline")
				actualConfig, _, teamPipelineVersion, err := team.GetConfig("steve")
				Expect(err).NotTo(HaveOccurred())
				...
				_, _, err = team.SavePipeline("steve", otherConfig, teamPipelineVersion, dbng.PipelineNoChange)
				Expect(err).NotTo(HaveOccurred())

				_, _, err = otherTeamDB.SavePipeline("steve", config, otherTeamPipelineVersion, dbng.PipelineNoChange)
				Expect(err).NotTo(HaveOccurred())

				actualOtherConfig, _, teamPipelineVersion, err = team.GetConfig("steve")
				Expect(err).NotTo(HaveOccurred())
				Expect(actualOtherConfig).To(Equal(otherConfig))
				...
				_, _, err = team.SavePipeline("steve", otherConfig, teamPipelineVersion, dbng.PipelinePaused)
				Expect(err).NotTo(HaveOccurred())

				pausedPipeline, found, err := team.GetPipelineByID("steve")
				Expect(err).NotTo(HaveOccurred())
				Expect(found).To(BeTrue())

				unpausedPipeline, found, err := otherTeamDB.GetPipelineByID("steve")
				Expect(err).NotTo(HaveOccurred())
				Expect(found).To(BeTrue())
				...
				_, _, err = team.SavePipeline("steve", otherConfig, otherTeamPipelineVersion, dbng.PipelineNoChange)
				Expect(err).To(HaveOccurred())

				_, _, err = team.SavePipeline("steve", otherConfig, otherTeamPipelineVersion, dbng.PipelinePaused)
				Expect(err).To(HaveOccurred())
			})
		})
	})
